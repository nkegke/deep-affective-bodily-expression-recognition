...
def main():
	...
	videos_ = [base + 'Data_mask/Validation_mask/', base + 'Data_mask/Train_mask/']
	labels_ = [base + 'Labels/val_labels.text', base + 'Labels/train_labels.text']
	files_output = [base + 'TSM/val_videofolder.txt', base + 'TSM/train_videofolder.txt']
	for (filenames, videos, labels, file_output) in zip(filenames_, videos_, labels_, files_output):
		...
		for line in lines[:-1]:
			...
			folders.append(folder[:-5] + '_mask_frames') #get rid of ".mp4\n"
